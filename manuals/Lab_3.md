...
Во вкладке `Состав` (`Details`) и `Путь сертификации` (`Certification Path`) можно посмотреть более подробную информацию о сертификате.

![](../images/lab_3/3.22.png)

![](../images/lab_3/3.23.png)

Для того, чтобы ОС доверяла нашему сертификату установим его в хранилище доверенных корневых сертификатов (кнопка `Установить сертификат...` (`Install Certificate...`) во вкладке `Общие` (`General`)):
...
Первая половина работы была о том, как защитить канал, по которому код попадает к пользователю. Вторая будет о том, как этот код хранить: чтобы можно было увидеть историю правок, вернуться к любой предыдущей версии и вести разработку нескольких функций параллельно. Кроме того, выпущенные только что SSH-ключи и умение работать с ними понадобятся в конце работы, при подключении к GitHub.
...
>- ветка — `master`;
>- автор и его мэйл — `batman <batman@gotham.com>`;
...
>[!WARNING]
>`b8f3546` — хэш из примера, у вас он будет другим. Возьмите идентификатор своего последнего коммита из вывода `git log`. Если не хочется переписывать хэш руками, тот же коммит можно указать относительно текущей позиции: `git revert HEAD`.
...
>
>Обратите внимание и на разницу в последствиях: `git revert` ничего не стирает и потому безопасен для веток, которые уже опубликованы, а `git reset --hard` переписывает историю и безвозвратно удаляет незакоммиченные изменения в рабочем каталоге. На общих ветках второй способ применять не стоит.
...
И еще добавим порт `8080` в файрволл:
...
Возникла ошибка - конфликт, потому что строка `<div class="matrix" id="matrixCode"></div>` которую мы добавили в `html_base` находится там же, где открывающий тег `<script>` в ветке `script`. Из-за этого возникает исключение, так как Git не понимает, какую строчку нужно оставить, а какую нет. Поэтому он возлагает эту задачу на нас.
...
Можем заметить появившиеся строки с `<<<<<<< HEAD`, `=======` и `>>>>>>> script`. Это автоматически генерируемые Git строки, указывающие на область конфликта: между `<<<<<<< HEAD` и `=======` находится вариант из текущей ветки (`html_base`), между `=======` и `>>>>>>> script` — вариант из вливаемой ветки (`script`). Нам нужно оставить обе добавленные строки, а сами маркеры удалить. Удалим их и приведём файл к состоянию как на скриншоте ниже:
...
>[!NOTE]
>Здесь `git commit` намеренно вызывается без `-m`: при завершении слияния Git сам подставляет текст сообщения (`Merge branch 'script' into html_base`) и открывает редактор, чтобы его можно было дополнить. Оставляем как есть и выходим из Vi так же, как раньше — `:wq`.
...
Зайдём на страничку [GitHub](https://github.com) и перейдём в наш аккаунт, если такого у вас нет, то надо его создать, нажав на `Sign up`.
...
После этого нас перенаправит на страницу репозитория. Нажмем на `SSH`. И GitHub любезно предоставит нам команды для подключения нашего локального репозитория к удалённому.
